...
4)  One possitble manner by which to break the code is make the value for fileNA larger than the amount of rows (in this case, fileSize=c(15,100) so I altered fileNA to 16. This ended up breaking the program resulting in the following error:
...
pVal <- rep(NA,nFiles)
r2 <- rep(NA,nFiles)
oriRows <- rep(NA, nFiles)                  # NEW CODE
cleanRows <-rep(NA, nFiles)                # NEW CODE

statsOut <- data.frame(ID,fileName,slope,pVal,r2, oriRows, cleanRows)                # NEW CODE
...
for (i in seq_along(fileNames)) {
  ...
  . <- regStats(dClean) # pull regression stats from clean file
  statsOut[i,3:5] <- unlist(.) # unlist, copy into last 3 columns
  statsOut[i,6] <- nrow(data)                # NEW CODE
  statsOut[i,7] <- nrow(dClean)                  # NEW CODE
}
# set up output file and incorporate time stamp and minimal metadata
write.table(cat("# Summary stats for ",
                "batch processing of regression models","\n",
